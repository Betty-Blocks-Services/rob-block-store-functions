...
const dataExport = async ({
  ...
  const getValue = (prop, obj) => {
    ...
    const value = prop.split('.').reduce((result, key, index, keys) => {
      if (result === null || result === undefined) {
        return null;
      }

      // Check if key exists in result (handles false, 0, empty string, etc.)
      if (!(key in result)) {
        return null;
      }
    ...
    if (typeof value === 'boolean') {
      // For CSV, convert to string; for XLSX, keep as boolean
      return value;
    }

    // Handle formatting if specified
    if (
      valueFormat &&
      valueFormat[0] &&
      valueFormat[1] &&
  ...
  const query = `
    query {
      all${modelNameSource}(${queryFilter} ${sortFilter} skip: $skip, take: $take) {
        results
  ...
  try {
    if (type == 'csv') {
      ...
      // // Set boolean cell types for columns that contain boolean values
      // const range = XLSX.utils.decode_range(ws['!ref'] || 'A1');
      // const booleanColumns = new Set();

      // // First pass: identify columns with boolean values
      ...
      // // Second pass: set cell type to 'b' (boolean) for identified columns
      // for (let row = range.s.r + 1; row <= range.e.r; row++) {
      //   for (const col of booleanColumns) {
      //     const cellAddress = XLSX.utils.encode_cell({ r: row, c: col });
      //     const cell = ws[cellAddress];
      //     if (cell && typeof cell.v === 'boolean') {
      //       cell.t = 'b'; // Set cell type to boolean
      //     }
      //   }
      // }
